...
class ViewSpecificTeacherMakeUpClass extends StatefulWidget {
  const ViewSpecificTeacherMakeUpClass({Key? key}) : super(key: key);

  @override
  State<ViewSpecificTeacherMakeUpClass> createState() => _ViewSpecificTeacherMakeUpClassState();
}

class _ViewSpecificTeacherMakeUpClassState extends State<ViewSpecificTeacherMakeUpClass> {

  void initState() {
      super.initState();
      ShowTeacherMakeUpClass();
    }

    List<MakeUpClass> makeupclassTable = [];

    Future<void> ShowTeacherMakeUpClass() async{
    var response=await http.get(Uri.parse(ApiUrl.apiUrl + 'meyepro/api/MakeUpClass/getTeacherMakeUpClass?teachername='+name1.toString()));
    if(response.statusCode==200)
    {
      print("Successfull Show Record");

      Iterable listt = jsonDecode(response.body);
      makeupclassTable = listt.map((e) => MakeUpClass.fromJson(e)).toList();

      if(listt.isEmpty)
      {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(name1.toString() + " Record Not Found!"),));
      }
  ...
  Widget build(BuildContext context) {
    ...
    timelist = makeupclassTable.where((element) => (element.stime == stime && element.etime == etime)).toList();
                        ...
                        Expanded(
                          child: Container(
                            ...
                            child: !_folded ? TextField(
                              onChanged: (value){
                                setState(() {
                                  ListOnSearch = makeupclassTable.where((element) => 
                                    (
                              ...
                              decoration: InputDecoration(
                                hintText: 'Search From MakeUp Class',
                    ...
                    child: _textEditingController!.text.isNotEmpty &&
                        ...
                        : Container(
                            ...
                            child: Expanded(
                              ...
                              child: makeupclassTable.isNotEmpty ? DataTable(
                                columnSpacing: 38.0,
                                ...
                                rows: List.generate(_textEditingController!.text.isNotEmpty ? ListOnSearch.length : makeupclassTable.length, (index) {
                                  final sectionname = _textEditingController!.text.isNotEmpty ? ListOnSearch[index].sectionname : makeupclassTable[index].sectionname;
                                  // final tid = _textEditingController!.text.isNotEmpty ? ListOnSearch[index].mcid.toString() : makeupclassTable[index].mcid.toString();
                                  final day = _textEditingController!.text.isNotEmpty ? ListOnSearch[index].dayy : makeupclassTable[index].dayy.toString();
                                  final teacher =  _textEditingController!.text.isNotEmpty ? ListOnSearch[index].teachername : makeupclassTable[index].teachername.toString();
                                  final subject = _textEditingController!.text.isNotEmpty ? ListOnSearch[index].coursename : makeupclassTable[index].coursename.toString();
                                  final venue = _textEditingController!.text.isNotEmpty ? ListOnSearch[index].venue : makeupclassTable[index].venue.toString();
                                  final time = (_textEditingController!.text.isNotEmpty ? ListOnSearch[index].stime : makeupclassTable[index].stime.toString()) +
                                               ' - ' + 
                                               (_textEditingController!.text.isNotEmpty ? ListOnSearch[index].etime : makeupclassTable[index].etime.toString());
                              ...
                              Center(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      name1.toString() + ' Record Not Found!',
                                      style: cardtextstyle,
                                    ),
                                  ],
                                ),
