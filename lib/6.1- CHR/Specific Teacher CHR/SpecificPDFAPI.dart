...
import '../../main.dart';
import 'SpecificTeacherCHR2.dart';
...
class SpecificPdfApi {

  static Future<File> generateTable() async {
    var response=await http.get(Uri.parse(ApiUrl.apiUrl + 'meyepro/api/CHR2/getTeacherCurrentDayCHR2?teachername=' + name1.toString() + '&indate=' + datef.toString()));
    ...
    if (response.statusCode == 200) {
      ...
      var data = listt.map((e) => TaskCHRTable.fromJson(e)).toList();
      ...
      pdf.addPage(MultiPage(
        ...
        build: (context) => <Widget>[
          ...
          Center(
            child: Text('Class Held Report ${datef.toString()}', 
              style: TextStyle(color: PdfColors.blue,letterSpacing: 1,fontWeight: FontWeight.bold))
              ),
          SizedBox(height: 15),
          ...
          data: <List<String>>[
            for (int i = 0; i < listt.length; i++)
              <String>[
                ...
                '${data.elementAt(i).status}',
                '${data.elementAt(i).latein}',
                '${data.elementAt(i).leftearly}'
              ],
          ],
        ),
        ]
      ...
      // }
      return saveDocument(name: 'chr_${DateFormat("yyyy-MM-dd").format(DateTime.now())}.pdf', pdf: pdf);
    } else {
  ...
  static Future<File> generateTable2() async {
    ...
    var data1 = listt1.map((e) => TaskCHRTable.fromJson(e)).toList();
    var data2 = listt2.map((e) => TaskCHRTable.fromJson(e)).toList();
    var data3 = listt3.map((e) => TaskCHRTable.fromJson(e)).toList();
    var data4 = listt4.map((e) => TaskCHRTable.fromJson(e)).toList();
    var data5 = listt5.map((e) => TaskCHRTable.fromJson(e)).toList();
