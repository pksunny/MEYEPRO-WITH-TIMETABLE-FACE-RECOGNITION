...
import 'package:m_eye_gui/6.1-%20CHR/Specific%20Teacher%20CHR/SpecificPDFAPI.dart';
...
class SpecificTeacherCHR2 extends StatefulWidget {
  const SpecificTeacherCHR2({Key? key}) : super(key: key);

  @override
  _SpecificTeacherCHR2State createState() => _SpecificTeacherCHR2State();
}

class _SpecificTeacherCHR2State extends State<SpecificTeacherCHR2> {
  ...
  Future<void> ShowSpecificCHR2() async{
    var datef = DateFormat("yyyy-MM-dd").format(DateTime.now());
    print(datef);
    var response=await http.get(Uri.parse(ApiUrl.apiUrl + 'meyepro/api/CHR2/getTeacherCurrentDayCHR2?teachername=' + name1.toString() + '&indate=' + datef));
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Expanded(
                      ...
                      child: chrTable.isNotEmpty ? DataTable(
...
class SpecificTeacherCurrentDateCHR2 extends StatefulWidget {
  const SpecificTeacherCurrentDateCHR2({Key? key}) : super(key: key);

  @override
  _SpecificTeacherCurrentDateCHR2State createState() => _SpecificTeacherCurrentDateCHR2State();
}

class _SpecificTeacherCurrentDateCHR2State extends State<SpecificTeacherCurrentDateCHR2> {
  ...
  void initState() {
    ...
    // var dt = DateFormat('EEEE').format(DateTime.parse(datef.toString())) + "00:10:00";
    // var cdt = DateFormat('EEEE').format(DateTime.parse(datef.toString()));

    // cdt == dt ? cdt : dt.toString(); 
  }
  ...
  Future<void> ShowSpecificCHR2() async{
    ...
    if(response.statusCode==200)
    {
      ...
      {
        var indate = chrTable[i].indate.toString();
      }
    }
    else
      throw Exception('Failed to load data');
    ...
    return Scaffold(
      ...
      bottomNavigationBar: ConvexAppBar(
        ...
        onTap: (int i) async{
          if(i == 0)
          {
            final pdfFile = await SpecificPdfApi.generateTable();
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(height: 15),
                  datef!.isNotEmpty 
                      ? Text('{${DateFormat('EEEE').format(DateTime.parse(datef.toString()))}}', style: TxtStyle.cardtextstyle2) 
                      : Text('{invalid date}'),
                  ...
                  SizedBox(height: 15,),
                  Text('Class Held Report ${datef}',style: TxtStyle.boldtxtstyleblue,),
